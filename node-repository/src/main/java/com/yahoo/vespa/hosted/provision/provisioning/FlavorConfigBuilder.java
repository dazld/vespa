...
public class FlavorConfigBuilder {
    ...
    public FlavorsConfig.Flavor.Builder addFlavor(String flavorName, double cpu, double mem, double disk, FlavorType type) {
    ...
    public FlavorsConfig.Flavor.Builder addNonStockFlavor(String flavorName, double cpu, double mem, double disk, FlavorType type) {
        FlavorsConfig.Flavor.Builder flavor = new FlavorsConfig.Flavor.Builder();
        flavor.name(flavorName);
        flavor.description("Flavor-name-is-" + flavorName);
        flavor.minDiskAvailableGb(disk);
    ...
    public static NodeFlavors createDummies(String... flavors) {
        ...
        for (String flavorName : flavors) {
            if (flavorName.equals("docker"))
                flavorConfigBuilder.addFlavor(flavorName, 1. /* cpu*/, 3. /* mem GB*/, 2. /*disk GB*/, FlavorType.DOCKER_CONTAINER);
            else if (flavorName.equals("docker2"))
                flavorConfigBuilder.addFlavor(flavorName, 2. /* cpu*/, 4. /* mem GB*/, 4. /*disk GB*/, FlavorType.DOCKER_CONTAINER);
            else if (flavorName.equals("host"))
                flavorConfigBuilder.addFlavor(flavorName, 7. /* cpu*/, 10. /* mem GB*/, 12. /*disk GB*/, FlavorType.BARE_METAL);
            else
                flavorConfigBuilder.addFlavor(flavorName, 1. /* cpu*/, 3. /* mem GB*/, 2. /*disk GB*/, FlavorType.BARE_METAL);
        }
