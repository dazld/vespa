...
public class Dispatcher extends AbstractComponent {
    ...
    public Optional<FillInvoker> getFillInvoker(Result result, VespaBackEndSearcher searcher) {
        Optional<FillInvoker> rpcInvoker = rpcInvokerFactory.createFillInvoker(result, searcher);
        if (rpcInvoker.isPresent()) {
            return rpcInvoker;
        }
        if (result.getQuery().properties().getBoolean(dispatchInternal, internalDispatchByDefault)) {
            Optional<FillInvoker> fs4Invoker = fs4InvokerFactory.createFillInvoker(result, searcher);
    ...
    private Optional<SearchInvoker> getSearchPathInvoker(Query query, InvokerFactory invokerFactory, VespaBackEndSearcher searcher) {
        ...
        try {
            ...
            if (nodes.isEmpty()) {
                ...
                return invokerFactory.createSearchInvoker(query, searcher, OptionalInt.empty(), nodes, true);
            }
    ...
    private Optional<SearchInvoker> getInternalInvoker(Query query, InvokerFactory invokerFactory, VespaBackEndSearcher searcher) {
        ...
        if (directNode.isPresent()) {
            ...
            return invokerFactory.createSearchInvoker(query, searcher, OptionalInt.empty(), Arrays.asList(node), true);
        }
        ...
        for (int i = 0; i < max; i++) {
            ...
            Optional<SearchInvoker> invoker = invokerFactory.createSearchInvoker(query, searcher, OptionalInt.of(group.id()), group.nodes(),
