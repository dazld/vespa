...
public abstract class InvokerFactory {
    public abstract Optional<SearchInvoker> createSearchInvoker(Query query, VespaBackEndSearcher searcher, OptionalInt groupId,
            List<Node> nodes, boolean acceptIncompleteCoverage);

    public abstract Optional<FillInvoker> createFillInvoker(Result result, VespaBackEndSearcher searcher);

    protected SearchInvoker createCoverageErrorInvoker(List<Node> nodes, Set<Integer> failed) {
