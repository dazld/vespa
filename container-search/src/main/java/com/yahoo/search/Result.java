...
public final class Result extends com.yahoo.processing.Response implements Cloneable {
    ...
    public static Result fromProtobuf(Query query, Searchprotocol.Result protobuf, DocumentDatabase documentDatabase) {
        ...
        for (var document : protobuf.getDocumentList()) {
            FastHit hit = new FastHit();
            hit.setQuery(query);

            hit.setRelevance(new Relevance(document.getRelevance()));
            hit.setGlobalId(new GlobalId(document.getGlobalId().toByteArray()));

            hit.setFillable();
            // XXX hit.setCacheKey(cacheKey);
            // XXX hit.setCached(fromCache);
    ...
    public Searchprotocol.Result toProtobuf() {
        ...
        if(coverage != null) {
            builder.setCoverageDocs(coverage.getDocs())
                ...
                .setNodesQueried(coverage.getNodesTried())
                .setNodesReplied(coverage.getNodes());
            if (coverage.isDegradedByAdapativeTimeout())
                builder.addDegradedReason(DegradedReason.DEGRADED_BY_ADAPTIVE_TIMEOUT);
            if (coverage.isDegradedByMatchPhase())
                builder.addDegradedReason(DegradedReason.DEGRADED_BY_MATCH_PHASE);
            if (coverage.isDegradedByTimeout())
                builder.addDegradedReason(DegradedReason.DEGRADED_BY_TIMEOUT);
        }
        ...
        hits.iterator().forEachRemaining(hit -> {
            ...
            if (hit instanceof FastHit) {
                ...
                docBuilder.setGlobalId(ByteString.copyFrom(fhit.getGlobalId().getRawId()));
            }
