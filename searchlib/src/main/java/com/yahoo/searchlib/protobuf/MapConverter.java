...
public class MapConverter {
    ...
    public static void convertMapStrings(Map<String, Object> map, PropertyInserter<StringProperty.Builder> inserter) {
        for (var entry : map.entrySet()) {
            var value = entry.getValue();
            if (!(value instanceof Tensor)) {
                inserter.add(StringProperty.newBuilder().setName(entry.getKey()).setValue(value.toString()));
            }
        }
    }
        ...
        for (var entry : map.entrySet()) {
            ...
            if (values != null) {
                ...
                for (var value : values) {
                    inserter.add(StringProperty.newBuilder().setName(key).setValue(value));
                }
            }
        }
    }
                ...
                for (var value : entry.getValue()) {
                    if (value != null) {
                        if (value instanceof Tensor) {
                            ...
                            stringInserter.add(StringProperty.newBuilder().setName(key).setValue(value.toString()));
                        }
                    }
                }
            }
